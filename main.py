...
from components.charts import (create_shipping_timeline,
...
def main():
    st.set_page_config(page_title="Shipping Dashboard",
                       page_icon="🚢",
                       layout="wide")
    ...
    with col1:
        if st.session_state.authenticated:
            ...
            st.write("Welcome, Guest! This Is Brian's Shipping Dashboard.")
    with col2:
        if st.session_state.authenticated:
    ...
    if not st.session_state.authenticated and st.session_state.get(
            'show_login', False):
        login_form()
    ...
    if st.session_state.current_page == "Add Entry":
        ...
        with col1:
            # Timeline chart
            st.plotly_chart(create_shipping_timeline(
                get_date_range(entries, 30)),
                            use_container_width=True)

        with col2:
            # Category distribution
            st.plotly_chart(create_category_distribution(entries),
                            use_container_width=True)

        # Shipping frequency
        st.plotly_chart(create_shipping_frequency(entries),
                        use_container_width=True)
